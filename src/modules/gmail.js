    ...
    const storeMessage = (message) => {
      ...
      return new Promise((resolve, reject) => {
        if (message) {
          // Download the file
          // Path to the file
          const downloadFilePath = path.normalize(`./.cache/${fileName}.txt`)
          // Create the file
    ...
    const storeAttachment = (attachment) => {
      ...
      return new Promise((resolve, reject) => {
        if (attachment) {
          // Download the file
          // Path to the file
          const downloadFilePath = appendExtToFileName(path.normalize(`./.cache/${fileName} - ${attachment.fileName}`), attachment.mimeType)
          // Create the file
          fs.createFile(downloadFilePath)
          .then(() => {
            // Decode the bytes into a buffer
