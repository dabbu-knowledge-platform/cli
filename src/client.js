...
const { get, set, getAbsolutePath, refreshAccessToken, generateBodyAndHeaders, printInfo, printBright, printFiles, getExtForMime, escapeRegex, printError, parseUserInputForPath } = require("./utils")
...
const listRequest = async (drive, folderPath, regex) => {
  ...
  let encodedFolderPath = encodeURIComponent(folderPath === "" ? "/" : folderPath)
...
const downloadRequest = async (drive, folderPath, fileName) => {
  ...
  if (file && file.contentURI) {
    ...
    if (file.contentURI.startsWith("file://")) {
      ...
      res = fs.createReadStream(unescape(file.contentURI).replace("file://", ""))
...
const uploadRequest = async (drive, folderPath, fileName, localPath) => {
  ...
  formData.append("content", fs.createReadStream(localPath), { filename: fileName })
...
const updateRequest = async (drive, fromFolderPath, fromFileName, toFolderPath, toFileName) => {
  // Generate request body and headers
  let [body, headers] = generateBodyAndHeaders(drive)
  ...
  let server = get("server")
  let provider = get(`drives.${drive}.provider`)
  let encodedFolderPath = encodeURIComponent(fromFolderPath === "" ? "/" : fromFolderPath)
...
const Client = class {
  ...
  async list(args) {
    ...
    let {drive, folderPath, regex} = await parseUserInputForPath(args[1], true, null, "dabbu_fallback_file_id")

    spinner.text = `Loading all files ${chalk.blue(regex ? `matching regex ${regex}` : `in folder ${folderPath}`)}`
  ...
  async read(args) {
    ...
    // Get the path the user entered, default to current directory
    let {drive, folderPath, fileName} = await parseUserInputForPath(args[1], false)
    ...
    printInfo(`File downloaded ${chalk.keyword("orange")("temporarily")} to ${localPath}`)
  ...
  async copy(args) {
    // Show a loading indicator
    const spinner = ora(`Copying your ${chalk.blue("files and folders")}`).start()

    // Get the path the user entered (the file(s) to copy)
    let {drive: fromDrive, folderPath: fromFolderPath, fileName: fromFileName, regex: fromRegex} = await parseUserInputForPath(args[1], true)
    // Get the path the user entered (the target to copy to)
    let {drive: toDrive, folderPath: toFolderPath, fileName: toFileName} = await parseUserInputForPath(args[2], false, null, fromFileName || "dabbu_fallback_file_id")
    ...
    if (fromRegex) {
      ...
      if (files && files.length > 0) {
        ...
        for (let i = 0, length = files.length; i < length; i++) {
          ...
          try {
            ...
            if (file.kind === "folder") {
              throw new Error("Copying/moving/downloading folders is not yet supported, only deleting is")
            }
            // Fetch the file
            let localPath = await downloadRequest(fromDrive, fromFolderPath, file.name)
            // Upload the file
            let res = await uploadRequest(toDrive, toFolderPath, file.name, localPath)
            // Tell the user
            spinner.stop()
            printInfo(`Copied file ${fromFolderPath}/${file.name} to ${toFolderPath}/${file.name}`)
            spinner.start()
          } catch (err) {
      ...
      try {
        ...
        // Fetch the file
        let localPath = await downloadRequest(fromDrive, fromFolderPath, fromFileName)
        // Upload the file
        let res = await uploadRequest(toDrive, toFolderPath, toFileName, localPath)
        // Tell the user
        spinner.stop()
        printInfo(`Copied file ${fromFolderPath}/${fromFileName} to ${toFolderPath}/${toFileName}`)
      } catch (err) {
        spinner.stop()
        throw err
      }
  ...
  async move(args) {
    // Show a loading indicator
    const spinner = ora(`Moving your ${chalk.blue("files and folders")}`).start()
    ...
    if (fromRegex) {
      ...
      if (files && files.length > 0) {
        ...
        for (let i = 0, length = files.length; i < length; i++) {
          ...
          // Update the spinner
          spinner.text = `Moving file ${fromFolderPath}/${fromFileName || file.name} to ${toFolderPath}/${toFileName || file.name}`
          ...
          if (fromDrive === toDrive) {
            ...
            try {
              ...
              toFileName = toFileName.replace("dabbu_fallback_file_id", fromFileName || file.name)
              let res = await updateRequest(fromDrive, fromFolderPath, fromFileName || file.name, toFolderPath, toFileName || file.name)
              // Tell the user
              spinner.stop()
              printInfo(`Moved file ${fromFolderPath}/${fromFileName || file.name} to ${toFolderPath}/${toFileName || file.name}`)
              spinner.start()
            } catch (err) {
              // Print the error, but continue
              spinner.stop()
            ...
            try {
              ...
              let localPath = await downloadRequest(fromDrive, fromFolderPath, file.name)
              // Upload the file
              let res = await uploadRequest(toDrive, toFolderPath, toFileName || file.name, localPath)
              // Tell the user
              spinner.stop()
              printInfo(`Moved file ${fromFolderPath}/${file.name} to ${toFolderPath}/${toFileName || file.name}`)
              spinner.start()
            } catch (err) {
              // Print the error, but continue
              spinner.stop()
      ...
      try {
        ...
        if (fromDrive === toDrive) {
          // Update the file
          // Check if the fallback name needs to be replaced
          toFileName = toFileName.replace("dabbu_fallback_file_id", fromFileName)
          let res = await updateRequest(fromDrive, fromFolderPath, fromFileName, toFolderPath, toFileName)
          // Tell the user
          spinner.stop()
          printInfo(`Moved file ${fromFolderPath}/${fromFileName} to ${toFolderPath}/${toFileName}`)
        } else {
          // Fetch the file
          let localPath = await downloadRequest(fromDrive, fromFolderPath, fromFileName)
          // Upload the file
  ...
  async delete(args) {
    ...
    // Get the path the user entered, default to current directory
    let {drive, folderPath, fileName, regex} = await parseUserInputForPath(args[1], true, null, "dabbu_fallback_file_id")

    spinner.text = `Deleting ${chalk.blue(regex ? `all files matching regex ${regex}` : `file ${folderPath}/${fileName ? fileName : ""}`)}`
    ...
    spinner.stop()
    // Tell the user
    printInfo(`Deleted ${chalk.blue(regex ? `all files matching regex ${regex}` : `file ${folderPath}/${fileName ? fileName : ""}`)}`)
    // Return successfully
    return
  }
}
